...
namespace LidarrCompanion.Helpers
{
    ...
    public static class FileOperationsHelper
    {
        ...
        /// <summary>
        /// Resolves a queue-record file path that could sit under either of two known Lidarr
        /// locations: its import path, or (if configured) a separate download-client path. Tries
        /// the import path mapping first; if that prefix doesn't match the input (ResolveMappedPath
        /// returns the path unchanged when the configured prefix doesn't apply), falls back to the
        /// download path mapping. If DownloadPathLidarr isn't configured, this is equivalent to
        /// resolving against ImportPathLidarr alone.
        /// </summary>
        public static string ResolveMappedPathAnyKnown(string filePath, bool serverToLocal)
        {
            var viaImportPath = ResolveMappedPath(filePath, SettingKey.ImportPathLidarr, serverToLocal);
            if (!string.Equals(viaImportPath, filePath, StringComparison.Ordinal))
                return viaImportPath;

            return ResolveMappedPath(filePath, SettingKey.DownloadPathLidarr, serverToLocal);
        }
