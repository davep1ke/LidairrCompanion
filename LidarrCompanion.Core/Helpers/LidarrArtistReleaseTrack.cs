...
namespace LidarrCompanion.Helpers
{
    ...
    public class LidarrArtistReleaseTrack : INotifyPropertyChanged
    {
        ...
        private bool _releaseHasOtherAssigned = false;
        private string _albumType = string.Empty;
        ...
        // A cached entry (see the background prefetch queue) can be displayed for the same
        // artist more than once across a session; handing out the same mutable instance each
        // time would let IsAssigned/Score changes from one display bleed into the next.
        public LidarrArtistReleaseTrack Clone() => new()
        {
            Release = Release,
            Track = Track,
            HasFile = HasFile,
            ...
            ReleaseHasOtherAssigned = ReleaseHasOtherAssigned,
            AlbumType = AlbumType,
            Score = Score
        };
    }
