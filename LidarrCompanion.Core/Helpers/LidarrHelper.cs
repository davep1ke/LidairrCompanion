...
namespace LidarrCompanion.Helpers
{
    public class LidarrHelper
    {
        ...
        // Preserves the original zero-arg usage (MatchingService and others still `new LidarrHelper()`).
        public LidarrHelper() : this(new HttpClient())
        {
        }

        // DI-friendly overload - lets a host (e.g. IHttpClientFactory in ASP.NET Core) supply
        // and own the HttpClient instead of a new one being created per LidarrHelper.
        public LidarrHelper(HttpClient client)
        {
        ...
        public async Task<LidarrArtist?> CreateArtistAsync(string artistJson)
        {
            ...
            try
            {
                ...
                // rootFolderPath, qualityProfileId, metadataProfileId and monitored are not available in the raw lookup JSON,
                // so fall back to the configured defaults (previously hardcoded here).
                ...
                return await CreateArtistAsync(artistName, foreignId, folder, rootFolder, qualityProfileId: qualityProfileId, metadataProfileId: metadataProfileId, monitored: true, searchForMissingAlbums: true).ConfigureAwait(false);
            }
            catch (Exception ex)
            {
        ...
        // Public rather than the original's `internal`: Core is now a separate assembly consumed
        // by host projects (Web, and eventually the retired WPF app), so `internal` would hide
        // this from all of them - it was only safe as internal when everything was one assembly.
        public async Task<bool> ImportFilesAsync(List<ProposedAction> proposedActions)
        {
